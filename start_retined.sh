...
directory(){
	[[ ! -d /home/socat ]] && mkdir -p /home/socat
	cd /home/socat
}

download(){
	wget https://github.com/dongxinb/kernel/raw/master/rinetd -O socat
	[[ ! -f socat ]] && echo -e "${Error} rinetd download failed, please check !" && exit 1
	chmod +x socat
}
...
config-rinetd(){
	...
	echo -e "#!/bin/bash \ncd /home/socat \nnohup ./socat -f -c config-port.conf raw ${IFACE} &" >> start.sh && chmod +x start.sh
}

self-start(){
	sed -i "s/exit 0/ /ig" /etc/rc.local
	echo -e "\n/home/socat/start.sh\c" >> /etc/rc.local
...
uninstall(){
	check_root
	kill -9 `ps -A | grep socat | awk '{print $1}'`
	rm -rf /home/socat
	iptables -t raw -F
	sed -i '/\/home\/socat\/start.sh/d' /etc/rc.local
