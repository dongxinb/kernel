...
installbbrplus(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N --no-check-certificate http://raw.githubusercontent.com/dongxinb/kernel/master/linux-headers-${kernel_version}.deb
		wget -N --no-check-certificate http://raw.githubusercontent.com/dongxinb/kernel/master/linux-image-${kernel_version}.deb
		dpkg -i linux-headers-${kernel_version}.deb
		dpkg -i linux-image-${kernel_version}.deb
		cd .. && rm -rf bbrplus
	fi
